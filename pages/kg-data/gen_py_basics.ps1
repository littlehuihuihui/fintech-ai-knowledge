...
# 按照现有知识图谱格式：id/title/level/content/children

$ErrorActionPreference = "Stop"

function Make-Leaf($id, $title, $level, $content) {
    return @{
        id = $id
        title = $title
        level = $level
...
function Make-Chapter($id, $title, $level, $intro, $leaves) {
    $leafList = @()
    foreach ($l in $leaves) { $leafList += $l }
    
    $leafTitles = ($leaves | ForEach-Object { $_.title }) -join "、"
...
"@
    ...
    $i = 1
    foreach ($l in $leaves) {
        $oneLine = if ($l.content -match "一句话定义[：:]\s*(.+?)(\n|$)") { $Matches[1].Substring(0, [Math]::Min(40, $Matches[1].Length)) } else { "..." }
        $content += "`n| $i | $($l.title) | $($l.level) | $oneLine |"
        $i++
    }
...
点开地图中的第 1 片绿色叶节点。本章合计约 **$leafCount** 片叶讲义。
"@
...
# --- 语法基础 ---
$leafVars = Make-Leaf "py-basic-variables" "变量与数据类型" "?" @"
### 课前
...
$leafOperators = Make-Leaf "py-basic-operators" "运算符与表达式" "?" @"
### 课前
...
$leafStrings = Make-Leaf "py-basic-strings" "字符串操作" "?" @"
### 课前
...
# --- 控制流 ---
$leafIf = Make-Leaf "py-basic-if" "条件判断 if" "?" @"
### 课前
...
$leafLoops = Make-Leaf "py-basic-loops" "循环 for/while" "?" @"
### 课前
...
$leafComprehensions = Make-Leaf "py-basic-comprehensions" "推导式" "??" @"
### 课前
...
# --- 函数 ---
$leafFuncDef = Make-Leaf "py-basic-func-def" "函数定义与调用" "?" @"
### 课前
...
$leafFuncArgs = Make-Leaf "py-basic-func-args" "参数与返回值" "??" @"
### 课前
...
$leafLambda = Make-Leaf "py-basic-lambda" "lambda 与高阶函数" "??" @"
### 课前
...
# --- 数据结构 ---
$leafList = Make-Leaf "py-basic-list" "列表 list" "?" @"
### 课前
...
$leafDict = Make-Leaf "py-basic-dict" "字典 dict" "?" @"
### 课前
...
$leafTupleSet = Make-Leaf "py-basic-tuple-set" "元组与集合" "?" @"
### 课前
...
$leafBuiltins = Make-Leaf "py-basic-builtins" "常用内置函数" "??" @"
### 课前
...
$chapterSyntax = Make-Chapter "py-basic-syntax" "语法基础" "?" "变量、数据类型、运算符、字符串" @(
    $leafVars, $leafOperators, $leafStrings
)

$chapterControlFlow = Make-Chapter "py-basic-control-flow" "控制流" "?" "条件判断、循环、推导式" @(
    $leafIf, $leafLoops, $leafComprehensions
)

$chapterFunctions = Make-Chapter "py-basic-functions" "函数" "?" "函数定义、参数、lambda" @(
    $leafFuncDef, $leafFuncArgs, $leafLambda
)

$chapterDataStructures = Make-Chapter "py-basic-data-structures" "数据结构" "?" "列表、字典、元组集合、内置函数" @(
    $leafList, $leafDict, $leafTupleSet, $leafBuiltins
)
...
$domainNode = @{
    ...
    children = @($chapterSyntax, $chapterControlFlow, $chapterFunctions, $chapterDataStructures)
}
...
$json = $serializer.Serialize($domainNode)

# 保存到临时文件
$tempPath = "D:\cursor\数据学习平台\kg-data\py-basics-temp.json"
[System.IO.File]::WriteAllText($tempPath, $json, [System.Text.Encoding]::UTF8)
Write-Output "Generated: $tempPath"
Write-Output "Size: $((Get-Item $tempPath).Length) bytes"
