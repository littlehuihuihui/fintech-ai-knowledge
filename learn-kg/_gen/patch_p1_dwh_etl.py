# -*- coding: utf-8 -*-
"""P1-3 DWH lakehouse + P1-4 ETL DQ engine."""
...
HUB = {"dwh": "hub-dwh.json", "etl": "hub-etl.json"}

# ---------- DWH lakehouse content ----------
DWH_SEC = r'''### 课前 · 章节导读
...
def patch_dwh_why_map(content: str) -> str:
    row = "| 3 | 湖仓一体 | ??? | 湖存文件，仓要治理；表格式补 ACID/演进/时间旅行 |"
    if "湖仓一体" not in content:
    ...
    # order
    if "湖仓一体" not in content.split("### 推荐顺序")[-1][:200]:
        content = content.replace(
            "```text\n定位与边界 → 主题域与集市\n```",
            "```text\n定位与边界 → 主题域与集市 → 湖仓一体\n```",
        )
        # fuzzy common patterns from enricher
        content = content.replace(
            "定位与边界 → 主题域与集市",
            "定位与边界 → 主题域与集市 → 湖仓一体",
        )
    content = content.replace("共 **2** 个直接下级", "共 **3** 个直接下级")
    content = content.replace("**2** 个直接下级", "**3** 个直接下级")
    return content


def patch_dwh_senior(content: str) -> str:
    # 定位里可能已有「湖仓一体」字样，仍要以必学顺序是否含 node_id 为准
    order = ""
    if "### 必学顺序" in content:
    ...
    if "5. 与 BI" in content:
        return content.replace(
            "5. 与 BI",
            "5. 湖仓一体（`dwh-lakehouse-sec`）  \n6. 与 BI",
            1,
        )
    if "### 验收口令" in content:
        return content.replace(
            "### 验收口令",
            "6. 湖仓一体（`dwh-lakehouse-sec`）：表格式、时间旅行、与分层落点\n\n### 验收口令",
            1,
        )
    if "### 验收口诀" in content:
        return content.replace(
    ...
    return content.rstrip() + "\n\n6. 湖仓一体（`dwh-lakehouse-sec`）\n"


def patch_etl_quality_map(content: str) -> str:
    row = "| 2 | 质量规则引擎 | ??? | 规则即配置；六维度；阻断 vs 告警 |"
    if "质量规则引擎" not in content:
        lines = content.splitlines()
        out = []
        inserted = False
        for line in lines:
            out.append(line)
            if (not inserted) and line.startswith("| 1 |") and "校验" in line:
                out.append(row)
                inserted = True
        content = "\n".join(out) + ("\n" if not content.endswith("\n") else "")
    content = content.replace(
    ...
    if "校验清单 → 质量规则引擎" not in content:
        content = content.replace("校验清单", "校验清单 → 质量规则引擎", 1)
    content = content.replace("共 **1** 个直接下级", "共 **2** 个直接下级")
    content = content.replace("**1** 个直接下级", "**2** 个直接下级")
    return content


def patch_etl_mid(content: str) -> str:
    if "质量规则引擎" in content or "etl-dq-engine-sec" in content:
        return content
    needle = "4. 主键唯一 → 源仓对账"
    if needle in content:
        return content.replace(
            needle,
            "4. 主键唯一 → 源仓对账 → 质量规则引擎（`etl-dq-engine-sec`）",
        )
    if "### 验收口诀" in content:
        return content.replace(
            "### 验收口诀",
            "4.5 质量规则引擎（`etl-dq-engine-sec`）：六维度 + YAML 断言 + 门禁分级\n\n### 验收口诀",
            1,
        )
    return content.rstrip() + "\n\n- 质量规则引擎（`etl-dq-engine-sec`）\n"
...
def main():
    ...
    if not why:
        raise SystemExit("dwh-why missing")
    for nid in (
        "dwh-lakehouse-sec",
        "dwh-lakehouse-intro",
        "dwh-table-format",
        ...
        ensure_unique(dwh, nid)

    sec = {
        "id": "dwh-lakehouse-sec",
        "title": "湖仓一体",
        "level": "???",
        "content": DWH_SEC.strip() + "\n",
        "lessonParent": True,
        "children": [
            {
                "id": "dwh-lakehouse-intro",
                "title": "湖仓一体直觉",
                "level": "???",
                "content": DWH_L1.strip() + "\n",
                "children": [],
            },
            {
                "id": "dwh-table-format",
                "title": "Iceberg/Hudi/Delta 对比",
                "level": "???",
                "content": DWH_L2.strip() + "\n",
                "children": [],
            },
            {
                "id": "dwh-lakehouse-boundary",
                "title": "湖仓边界与选型",
                "level": "???",
                "content": DWH_L3.strip() + "\n",
                "children": [],
            },
        ],
    }
    why.setdefault("children", []).append(sec)
    why["content"] = patch_dwh_why_map(why.get("content") or "")
    senior = find(dwh, "dwh-path-senior")
    if not senior:
        raise SystemExit("dwh-path-senior missing")
    # only path list — not a gold leaf body rewrite beyond required insert
    senior["content"] = patch_dwh_senior(senior.get("content") or "")
    sync("dwh", dwh)
    print("DWH lakehouse added under dwh-why")

    # ---- ETL ----
    etl = json.loads((KG / "etl.json").read_text(encoding="utf-8"))
    quality = find(etl, "etl-quality")
    if not quality:
        raise SystemExit("etl-quality missing")
    for nid in (
        "etl-dq-engine-sec",
        "etl-dq-dimensions",
        "etl-dq-rules",
        ...
        ensure_unique(etl, nid)

    dq = {
        "id": "etl-dq-engine-sec",
        "title": "质量规则引擎",
        "level": "???",
        "content": ETL_SEC.strip() + "\n",
        "lessonParent": True,
        "children": [
            {
                "id": "etl-dq-dimensions",
                "title": "质量六维度",
                "level": "??",
                "content": ETL_L1.strip() + "\n",
                "children": [],
            },
            {
                "id": "etl-dq-rules",
            ...
            },
            {
                ...
                "title": "质量门禁接入管道",
                "level": "???",
                "content": ETL_L3.strip() + "\n",
                "children": [],
            },
        ],
    }
    quality.setdefault("children", []).append(dq)
    quality["content"] = patch_etl_quality_map(quality.get("content") or "")
    mid = find(etl, "etl-path-mid")
    if not mid:
        raise SystemExit("etl-path-mid missing")
    mid["content"] = patch_etl_mid(mid.get("content") or "")
    sync("etl", etl)
    print("ETL dq engine added under etl-quality")
    ...
    for html in HTMLS:
        ...
        t2 = re.sub(
            r'const KG_DATA_VER = "[^"]+"',
            'const KG_DATA_VER = "20260922d"',
            t,
            count=1,
        )
        html.write_text(t2, encoding="utf-8")

    out = ROOT / "_gen" / "p1_half2_snippets.md"
    out.write_text(
        "\n\n".join(
            [
                "# P1 后半片段",
